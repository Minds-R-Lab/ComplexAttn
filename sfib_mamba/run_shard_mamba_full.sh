#!/usr/bin/env bash
# run_shard_mamba_full.sh -- main SHARD-for-Mamba evaluation on Mamba-2.8b
# (the model ROMBA used for their headline numbers). Runs CounterFact + zsRE
# at layer 39 (ROMBA's strongest W_o site) for 500 sequential edits.
#
# Expected wall time on H100: ~3-4 hours per cell for 500 edits at this scale.
...
echo
echo "================================================================"
echo " SHARD-for-Mamba summary @ N=500:"
echo "================================================================"
python - <<'PY'
import json, os
for path in [
    "results/counterfact_shard_mamba_mamba2_8b_seed0.json",
    "results/zsre_shard_mamba_mamba2_8b_seed0.json",
]:
    ...
    if r500 is None:
        print(f"{path}: (no N=500 entry)"); continue
    print(f"{path}: "
