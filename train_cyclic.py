"""
Training loop for the Z/n cyclic-rotation task.

Structurally identical to train_triple.py but takes a CyclicTaskSpec
to drive the data generator. n-class cross-entropy; tracks best-OOD
checkpoint and restores at the end.
"""
...
from data_cyclic import CyclicTaskSpec, make_dataset, make_depth_stratified_eval
...
def train_model(model, spec: CyclicTaskSpec, *,
    ...
    train_tok, train_lab, _ = make_dataset(spec, n_train, train_depth_range,
                                            seed=seed)
    eval_tok, eval_lab, eval_dep = make_depth_stratified_eval(
        spec, n_eval_per_depth, list(eval_depths), seed=seed + 10_000)
    ...
    best_ood, best_step, best_state = -1.0, 0, None
    ...
    for epoch in range(n_epochs):
        ...
        for tokens, y in train_dl:
            model.train()
            tokens, y = tokens.to(device), y.to(device).long()
            logits = model(tokens)
